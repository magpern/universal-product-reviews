...
2. **Milestone split.**  
   ...
   - **M10** — external processing requires a **separate freeze**; core must enforce external classification and opt-in before any outbound call; not provider self-declaration.  
...
4. **Privacy and secrets.** Star rating is excluded from provider inputs. Provider secrets are host-adapter-owned (environment or `wp-config.php` constants). UPR must never store provider API keys in options (plain or encrypted), audit, diagnostics, or support export. Host DPIA is a human/process obligation, not a machine-enforced core gate. External transmission is out of M9.
...
9. **Public contracts.** Do not register unimplemented AI provider surfaces in `upr-public-contracts/v1`. First registry entry occurs in the implementing milestone (local provider in M9; external in M10).
...
- Implementation PRs that introduce auto-approval, external HTTP in M9, option-stored secrets, or partial-index claim uniqueness are out of policy.
