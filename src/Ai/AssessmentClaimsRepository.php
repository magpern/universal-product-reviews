...
final class AssessmentClaimsRepository {
	...
	public static function ensure_row( int $comment_id, string $policy_version ): void {
		...
		$wpdb->query(
			$wpdb->prepare(
				"INSERT IGNORE INTO {$table} (comment_id, policy_version, claim_token, claim_expires_at, requested_at, updated_at)
				VALUES (%d, %s, NULL, NULL, %s, %s)",
				$comment_id,
				$policy_version,
				$now,
				$now
	 ...
	 */
	public static function acquire( int $comment_id, string $policy_version ): ?string {
		global $wpdb;
		...
		$n = $wpdb->query(
			$wpdb->prepare(
				"UPDATE {$table} SET claim_token = %s, claim_expires_at = %s, requested_at = %s, updated_at = %s
